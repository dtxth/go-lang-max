...
// SecurePasswordGenerator implements PasswordGenerator using crypto/rand
...
// Generate creates a cryptographically secure random password
// It ensures the password meets complexity requirements:
...
func (g *SecurePasswordGenerator) Generate(length int) (string, error) {
	...
	// Ensure we have at least 4 characters for the required character types
	if length < 4 {
...
// HasLowercase checks if the string contains at least one lowercase letter
...
// HasDigit checks if the string contains at least one digit
...
// HasSpecial checks if the string contains at least one special character
